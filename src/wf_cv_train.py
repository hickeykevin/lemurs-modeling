...
def wf_cv_train(cfg: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    reported (see ``src.utils.pooled_metrics.fold_breakdown_table``), to see
    whether forecasting improves as more history accumulates, but it is a
    ...
    # Every fold's callbacks are instantiated from the same cfg.callbacks, so
    # the last fold's ClassificationMetricsCallback instance (if any) is
    # representative of all of them -- reused here so pooled metrics use the
    # exact same averaging/threshold params, not a second, possibly-drifted
    # set of defaults.
    classification_metrics_callback = next(
        (cb for cb in callbacks if type(cb).__name__ == "ClassificationMetricsCallback"), None
    )
    pooled_metrics = compute_pooled_metrics(
        pooled_df, log, classification_metrics_callback=classification_metrics_callback
    )
    fold_breakdown = fold_breakdown_table(pooled_df, log)
