...
def test_train_eval(tmp_path: Path, cfg_train: DictConfig, cfg_eval: DictConfig) -> None:
    ...
    # train() returns an eval plan's aggregate (plain floats), while evaluate()
    # returns Lightning's callback_metrics (tensors) -- compare as floats rather
    # than assuming either type.
    train_f1 = float(train_metric_dict["test/f1_mean"])
    test_f1 = float(test_metric_dict["test/f1_mean"])

    if math.isnan(train_f1) or math.isnan(test_f1):
        ...
        assert math.isnan(train_f1) and math.isnan(test_f1)
    else:
        assert test_f1 > 0.0
        assert abs(train_f1 - test_f1) < 0.005
