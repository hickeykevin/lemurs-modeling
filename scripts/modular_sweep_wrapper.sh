#!/bin/bash
# Wrapper to translate modular W&B sweep parameters to Hydra config overrides safely.
...
for arg in "$@"; do
    case "$arg" in
        ...
        include_time_features_choice=*)
            val="${arg#*=}"
            ARGS="$ARGS \"data.include_time_features=$val\""
            ;;
...
eval "uv run python src/train.py \
    task_name=modular_hydra_sweep \
    paths.log_dir=logs/modular_hydra_sweep \
    ...
    'logger=[csv,wandb]' \
    logger.wandb.group=modular_hydra_sweep \
